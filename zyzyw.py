...
class CrawlerZYZYW(ShowStatus):

    def __init__(self):
        super(CrawlerZYZYW, self).__init__(SITE_NAME)
        self.treeIndex = 1
        self.headers = {
    ...
    def getPageIndexUrl(self, url):
        ...
        pageUrlList = [f"https://www.zhzyw.com{url}" for url in html.xpath("//div[@class='pagecontent']/a/@href") if "java" not in url]
    ...
    def indexPage(self, feature):
        ...
        urlList = ["https://www.zhzyw.com"+i for i in html.xpath("//div[@class='ullist01']//a/@href")]
    ...
    def startCrawler(self, start_url):
        pageUrlList = self.getPageIndexUrl(start_url)[:1]
        self.detailUrlList = []
        new_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(new_loop)
        self.loop = asyncio.get_event_loop()
