...
class CrawlerZYR(ShowStatus):

    def __init__(self):
        super(CrawlerZYR, self).__init__(SITE_NAME)
        self.treeIndex = 1
        self.headers = {
    ...
    def getPageIndexUrl(self, url):
        ...
        pageNum = ceil(int(html.xpath("//div[@class='showpage']/b[1]/text()")[0])/30)
        pageUrlList = [f"https://www.tcmer.com/Article/mjyf/gkyxf/List_{index}.html" for index in range(1, int(pageNum))]
    ...
    def indexPage(self, feature):
        ...
        urlList = ["https://www.tcmer.com"+i for i in html.xpath("//ul[@class='th_con_list']/table//a[contains(@title, '文章标题')]/@href")]
        # print(urlList)
    ...
    def detailPage(self, feature):
        result = feature.result()
        html = etree.HTML(result)
        title = html.xpath("//h1[@class='th_con_tit']/text()")[0]
