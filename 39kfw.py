...
class Crawler39KFW(ShowStatus):

    def __init__(self):
        super(Crawler39KFW, self).__init__(SITE_NAME)
        self.headers = {
    ...
    def getPageIndexUrl(self, url):
        ...
        totalPage = html.xpath("//div[@class='page']/text()")[0]
        pageNum = re.findall(r"分\s*(\d+)\s*页显示", totalPage)

        pageUrlList = [f"{url}/index{index}.shtml" for index in range(2, int(pageNum[0]) + 1)]
    ...
    def indexPage(self, feature):
        ...
        self.detailUrlList += html.xpath("//*[@id='TxtMain']/ul/li/a/@href")

    def detailPage(self, feature):
        result = feature.result()
        html = etree.HTML(result)
        title = html.xpath("//*[@id='Left']/h1/text()")[0]
...
if __name__ == '__main__':
    app = Crawler39KFW()
    app.start("http://www.39kf.com/focus/spaq/food-safety-knowledge/")
